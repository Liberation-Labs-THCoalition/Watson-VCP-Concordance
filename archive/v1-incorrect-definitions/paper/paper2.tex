...
In two new experiments on Qwen~2.5-7B and Llama-3.1-8B, we test per-layer mode-switching anatomy (Experiment~A, 96 trials) and controlled framing effects (Experiment~B, 80 paired trials). Five of six raw framing effects sign-flip after Frisch--Waugh--Lovell token-count residualization; the sixth (Llama eff\_rank) retains its sign but collapses to non-significance ($d_{\text{FWL}} = +0.38$, $p = 0.31$). Only spectral entropy survives in both architectures ($d_{\text{FWL}} = -1.92$ Qwen, $-0.71$ Llama, both $p < 0.001$). Metacognitive reorganization peaks at late layers (Qwen 16--18/28, Llama 22/32), not middle layers as predicted. Cognitive reversal is universal (93--94\% of layers in both models), but metacognitive reversal is architecture-specific (Qwen 86\%, Llama 0\%), explaining why Llama achieves the strongest canonical correlation ($r_1 = 0.76$): no mode-switch yields transparent VCP--geometry mapping.
...
This paper contributes six new findings:
...
\begin{enumerate}
    ...
    \item \textbf{FWL correction is non-negotiable for framing experiments} (Experiment~B): five of six raw framing effects sign-flip after token-count control; the sixth collapses to non-significance. Only spectral entropy survives universally.
    ...
    \item \textbf{Progressive spectral concentration during metacognitive generation} (Experiment~D): the mode switch is not instantaneous but emerges gradually, with top\_sv\_ratio divergence growing from $d = 0.45$ at token~10 to $d = 0.91$ at token~200.
\end{enumerate}
...
% ============================================================
% 9. Results VII: Temporal Dynamics (Experiment D)
% ============================================================
\section{Results VII: Temporal Dynamics of Mode-Switching}

\subsection{Experimental Design}

Experiments~A and~B establish that metacognitive framing reorganizes KV-cache geometry, but they compare only the final generation state. Does the mode switch happen immediately or does it emerge gradually during token generation? To answer this, we run 20 prompt pairs through Qwen~2.5-7B and Llama-3.1-8B, extracting KV-cache features at seven generation checkpoints (tokens 10, 25, 50, 75, 100, 150, 200).

\textbf{Critical design choice: length matching.} The original experiment revealed an input-length confound: metacognitive prompts contain 12--24 more input tokens than their cognitive counterparts, producing encode-phase Cohen's $d \approx 5.0$ before any generation begins. To eliminate this, we pad cognitive prompts with neutral instructional preambles (e.g., ``Please think carefully about the following question''), matching input token counts to within $\leq 2$ tokens per pair (mean difference: 0.2 tokens for Qwen, 0.25 for Llama).

\subsection{Validation: Length Matching Eliminates Token Confound}

For Qwen~7B, encode-phase effects drop to $|d| < 0.4$ for 5 of 6 features (all non-significant), confirming the length confound is eliminated. The exception is top\_sv\_ratio ($d = -1.12$, $p < .001$), which reflects genuine content sensitivity: different instructional text produces different spectral structure even at identical token counts. We address this residual encode difference through delta-from-encode analysis below.

Llama shows broader encode-phase sensitivity: 4 of 6 features show $|d| > 1.0$ despite matched token counts. This is not a failure of length matching---Llama's encode phase is simply more sensitive to prompt content. This architecture-dependent encoding sensitivity is itself a finding, consistent with the architecture-specific channels described in \Cref{sec:cca_loadings}.

\subsection{Progressive Divergence During Generation}

To isolate generation-phase effects from encode differences, we compute delta-from-encode trajectories: at each checkpoint, we subtract each prompt's own encode-phase feature value, yielding the change attributable to generation alone. If the mode switch is purely an encoding phenomenon, delta trajectories should be parallel between conditions.

They are not. For Qwen~7B, top\_sv\_ratio delta shows \textbf{progressive divergence}: metacognitive processing concentrates the spectrum more than cognitive processing at every checkpoint, and the effect \emph{grows} with generation depth:

\begin{table}[H]
\centering
\caption{Delta-from-encode Cohen's $d$ (metacognitive minus cognitive) for top\_sv\_ratio across generation checkpoints. Qwen~2.5-7B, $N = 20$ pairs.}
\label{tab:exp_d_trajectory}
...
The top\_sv\_ratio effect grows monotonically from $d = +0.45$ at token~10 to $d = +0.91$ at token~200. Metacognitive processing progressively concentrates the cache spectrum---pushing more variance into the leading singular value---as generation proceeds. This is not a prompt-encoding artifact: it emerges \emph{during} generation, absent from the encode phase.

Layer variance shows an opposing trajectory: metacognitive processing produces more uniform layer-wise norms (lower variance) during the first 50 tokens ($d = -0.89$), then partially relaxes. The early concentration in layer variance followed by relaxation, combined with the progressive concentration in top\_sv\_ratio, suggests a two-phase dynamics: metacognitive framing first homogenizes processing across layers (tokens 10--50), then progressively concentrates the spectral structure (tokens 50--200).

\subsection{Growth Rates Are Not Different}

Despite progressive divergence in the delta trajectories, per-token feature growth rates do not significantly differ between conditions (all $p > .05$). The divergence is not driven by different slopes but by different \emph{trajectories}: metacognitive generation follows a path that accumulates more spectral concentration per unit of content generated, even when the rate of change per token is similar.

\subsection{Cross-Model Consistency}

The cross-model $d$-profile correlation between Qwen and Llama is moderate ($\rho = 0.36$, $p = .02$, $n = 42$ feature-checkpoints). Both models show the same direction of effect for top\_sv\_ratio (metacognitive $>$ cognitive in delta trajectories), but the magnitude and feature profile differ---consistent with the architecture-dependent channels found throughout this paper.
...
\begin{enumerate}
    ...
    \item The mode switch is \textbf{progressive}, not instantaneous. Monitoring should track geometric trajectories across generation, not just final states. The strongest signal ($d = 0.91$) emerges only after 200 generated tokens.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Trajectory analysis uses padded prompts.} To match input token counts in Experiment~D, we prepend neutral instructional text to cognitive prompts. This eliminates the length confound but introduces a content difference at encoding. The delta-from-encode analysis controls for this, but cannot fully eliminate the possibility that different preamble text influences subsequent generation trajectories.
...
\begin{enumerate}
    ...
    \item The mode switch is \textbf{progressive}: metacognitive processing concentrates the cache spectrum gradually during generation ($d = +0.45$ at token~10 to $d = +0.91$ at token~200), not as an instantaneous switch at encoding time.
\end{enumerate}
